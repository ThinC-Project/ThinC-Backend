...
public class AnswerRecipe {
    ...
    public String answer2(@RequestBody Map<String,Object> request){
        ...
        if(globalProcess == null){
            ...
            Integer nextNum = num + 1;
            if((globalProcess.lastIndexOf(nextNum.toString() + ".") - 1)==-1){
                resultString = globalProcess.substring(globalProcess.lastIndexOf(num.toString() + ".") + 2);

            }
            else{
                resultString = globalProcess.substring(globalProcess.lastIndexOf(num.toString() + ".") + 2, globalProcess.lastIndexOf(nextNum.toString() + ".") - 1);
                globalProcess = globalProcess.substring(globalProcess.lastIndexOf(nextNum.toString()) -1);
            }
